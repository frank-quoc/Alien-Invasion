...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    ...
    ship.blitme()
    aliens.draw(screen) #draw function automatically draws each element in the group defined by rect value
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # Remove any bullets and alins that have collided
    ...
    if len(aliens) == 0:
        # DIf the entire fleet is destroyed, start a new level.
        bullets.empty()
...
def get_number_aliens_x(ai_settings, alien_width):
    """Determine the number of aliens the fit in a row."""
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    ...
    alien_width = alien.rect.width #get alien's width from rect attribute
    alien.x = alien_width + 2 * alien_width * alien_number #Each alien's position multiplied by 2 of it's with and pos
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    alien = Alien(ai_settings, screen) #let's us know alien's width and height. won't add
